...
def compute(plate, size, part):
    prev = copy.deepcopy(plate)
    for i in range(1, size):
        for j in range(1, size):
            plate[i][j] += computePoint(i, j, prev, size, part)

def computePoint(x, y, prev, size, part):
    dt = 0.05
...
def main():
    ...
    if rank == 0:
        ...
        for i in xrange(data[1]):
            ...
            comm.send(part2[data[0]-1], dest=4, tag=1)
            part2[data[0]] = comm.recv(source=4, tag=1)
            for j in xrange(data[0] + 1):
                part2[j][0] = col[j]
            # part2[data[0]-1][1] = temp
            compute(part2, data[0], rank)
            # part2[data[0]-1][1] = temp
            if i % 10 == 0:
        ...
        for i in xrange(data[1]):
            ...
            comm.send([row2[data[0]-1] for row2 in part3], dest=4, tag=2)
            comm.send(part3[1], dest=1, tag=1)
            col = comm.recv(source=4, tag=2)
            part3[0] = row
            for j in xrange(data[0] + 1):
                part3[j][data[0]] = col[j]
            # part3[1][data[0]-1] = temp
            compute(part3, data[0], rank)
            # part3[1][data[0]-1] = temp
            if i % 10 == 0:
        ...
        part4 = comm.recv(source=0)
        data = comm.recv(source=0)
        for i in xrange(data[1]):
            col = comm.recv(source=3, tag=2)
            row = comm.recv(source=2, tag=1)
            comm.send([row2[1] for row2 in part4], dest=3, tag=2)
            comm.send(part4[1], dest=2, tag=1)
            part4[0] = row
            for j in xrange(data[0] + 1):
                part4[j][0] = col[j]
            # part4[1][1] = temp
            compute(part4, data[0], rank)
            # part4[1][1] = temp
            if i % 10 == 0:
                file4 = open("four/file4_" + str(i) + ".txt", "wb")
                file4.write(show(part4, data[0]))
                file4.close()

    else:
       print "Expected only 5 nodes"
